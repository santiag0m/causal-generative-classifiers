...
class MDMMOptim:
    """
    Modified Differential Method of Multipliers based on:
    ...
    def lagrangian(
        ...
        constrained_loss: torch.Tensor,
        target_value: float,
    ) -> torch.Tensor:
        damp = self.damping * (target_value - constrained_loss.detach())
        return main_loss - (self.lambda_.weight - damp) * (
            target_value - constrained_loss
        )
    ...
    def step(self):
        ...
        if self.lambda_.weight < 0:
            self.lambda_.weight = nn.Parameter(torch.tensor(0.0))
